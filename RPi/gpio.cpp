...
{
	return (rpi_gpio_alt_function_t)((((rpi_reg_rw_t*)rpiGpio)[gpio / 10] >> ((gpio % 10) * 3)) & FS_MASK);
}
...
rpi_gpio_value_t RPI_GetGpioValue(rpi_gpio_pin_t gpio)
{
	...
	{
	case 0:
		result = (rpi_gpio_value_t)(rpiGpio->GPLEV0 & (1 << gpio));
		break;

	case 1:
		result = (rpi_gpio_value_t)(rpiGpio->GPLEV1 & (1 << (gpio - 32)));
		break;
	default:
		break;
	}
